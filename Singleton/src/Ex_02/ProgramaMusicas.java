...
import static Ex_02.FuncoesFicheiros.contarLinhasFicheiro;
import static Ex_02.FuncoesFicheiros.imprimirFicheiro;
...
public class ProgramaMusicas {
    ...
    public static void imprimirMusicasGenero(String genero, String[][] matrizCompleta) throws IOException {

        Logger loggerMusicas = Logger.getInstance("log.txt");
        loggerMusicas.log("Imprimir Música do Género: " + genero);

        System.out.println("***** Músicas de " + genero + " *****");
        for (int linha = 0; linha < matrizCompleta.length; linha++) {
            loggerMusicas.log("-*-*- Analisar: " + linha + "-*-*-");

            if (matrizCompleta[linha][2].equalsIgnoreCase(genero)) {
                loggerMusicas.log("! Música encontrada: "+matrizCompleta[linha][0]+"\t"+matrizCompleta[linha][1]+" !");
    ...
    public static void musicaMaisLonga(String[][] matrizCompleta) throws IOException {

        Logger logger = Logger.getInstance("logMusicas.txt");
        logger.log("Recebi a matriz completa");
        ...
        for (int linha = 0; linha < matrizCompleta.length; linha++) {
            ...
            logger.log("______ Analisar: " + linha + " _______");
            ...
            logger.log("Minutos atuais: " + minutosAtuais);
            logger.log("Segundos atuais: " + minutosAtuais + "\n");
    ...
    public static void musicasAcimaDeDuracao(String[][] matrizCompleta, int minutosPretendidos, int segundosPretendidos) throws IOException {

        Logger myLogger = Logger.getInstance("longestMusicaLog.txt");

        myLogger.log("Estou a começar a analisar a música mais longa");

        for (int linha = 0; linha < matrizCompleta.length; linha++) {

            myLogger.log("*** Analisar: " + linha + " ***");
            String[] duracaoDividida = matrizCompleta[linha][3].split(":");
            ...
            if (minutosAtuais > minutosPretendidos) { // Estamos perante a musica maior
                ...
                myLogger.log("Nome da Música: " + matrizCompleta[linha][0]);
            }
